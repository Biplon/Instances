...
public class GUIManager
{
    ...
    {
        Inventory inv = Bukkit.createInventory(null, 45, "IN:" + LanguageManager.joinWindowHeadText + " " + instance);
        for (int i = 0; i < 9; i++)
        {
    ...
    {
        Inventory inv = Bukkit.createInventory(null, 27, "IN: " + LanguageManager.leaveWindowHeadText);
    ...
    {
        Inventory inv = Bukkit.createInventory(null, 27, "IN: " + LanguageManager.readyWindowHeadText);
        createDefaultLayoutSmall(inv);
        setReady(inv);
        p.openInventory(inv);
    }
    ...
    {
        inv.setItem(12, createGuiItem(Material.GREEN_WOOL, LanguageManager.yesLeaveText));
        inv.setItem(14, createGuiItem(Material.RED_WOOL, LanguageManager.noLeaveText));
    }

    private void setReady(Inventory inv)
    {
        inv.setItem(12, createGuiItem(Material.GREEN_WOOL, LanguageManager.readyText));
        inv.setItem(14, createGuiItem(Material.RED_WOOL, LanguageManager.notReadyText));
    }
    ...
    private void setInstance(Inventory inv, String instance)
    {
        ...
        if (InstancesManager.getInstance().getPlayerInstanceConfig(instance).getInstances() != null)
        {
            for (PlayerInstance pi : InstancesManager.getInstance().getPlayerInstanceConfig(instance).getInstances())
            {
                if (!pi.isInUse())
                {
                    inv.setItem(count, createGuiItem(Material.GREEN_DYE, pi.getMyConfig().getInstanceName() + ":" + pi.getId(), pi.getMyGroup().getFullSlots() + "/" + pi.getMyGroup().getGroupSize() + " " + LanguageManager.playerText, LanguageManager.ownInvText + ": " + pi.getMyConfig().getPlayerOwnInventory()));
                }
                else
                {
                    inv.setItem(count, createGuiItem(Material.GREEN_DYE, pi.getMyConfig().getInstanceName() + ":" + pi.getId(), pi.getMyGroup().getFullSlots() + "/" + pi.getMyGroup().getGroupSize() + " " + LanguageManager.playerText, LanguageManager.ownInvText + ": " + pi.getMyConfig().getPlayerOwnInventory()));
